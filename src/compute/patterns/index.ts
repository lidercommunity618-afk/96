...
export function detectAllPatterns(
  ...
  // BUGFIX (аудит 2026-09-06, п.3): раньше это была computeStructure(candles, 60,
  // true, atrPeriod) — тот же M1-график с другим lookback, а не настоящий
  // старший таймфрейм (см. комментарий в htf-structure.ts). Теперь считается
  // один раз на реально ресэмплированных M15-барах и переиспользуется и
  // mean-reversion, и strong-order-block-reaction — обоим место, где раньше
  // либо был этот же псевдо-HTF, либо (strong-order-block-reaction) M1-локальный
  // ctx.structure выдавался за HTF bias напрямую.
  const needsHtfStructure = has('mean-reversion') || has('strong-order-block-reaction');
  const htfStructure = needsHtfStructure ? computeHtfStructure(candles, atrPeriod) : undefined;
